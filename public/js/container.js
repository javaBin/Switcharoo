...
import Info from './slide/info';
import Program from './slide/program';
import Twitter from './slide/twitter';
import Instagram from './slide/instagram';
import textTemplate from './slide/text-slide.hbs';
import imageTemplate from './slide/image-slide.hbs';
import programTemplate from './slide/program.hbs';
import twitterTemplate from './slide/twitter.hbs';
import instagramTemplate from './slide/instagram.hbs';
...
var view = Backbone.View.extend({
	...
	renderInfo: function() {
		...
		this.collection.each(function(slide) {
			...
			var template = slide.get('type') === 'image' ? imageSlideTemplate : textTemplate;
			var view = new Info.view({model: slide, template: template});
			slides.push(view);
			view.render();
		});
	...
	program: function() {
		...
		var model = new Program.model();
		var view = new Program.view({model: model, template: programTemplate});
	...
	twitter: function() {
		...
		var model = new Twitter.model();
		var view = new Twitter.view({model: model, template: twitterTemplate});
	...
	instagram: function() {
		...
		var model = new Instagram.model();
		var view = new Instagram.view({model: model, template: instagramTemplate});
	...
	start: function() {
		...
		Velocity(slide.animateIn(), 'transition.slideUpIn');
	...
	startTimers: function() {
		...
		setInterval(function() { self.getTwitter = true; }, TWO_MINUTES);
		setInterval(function() { self.getInstagram = true; }, TWO_MINUTES);
	},
	...
	slideInNext: function() {
        ...
        function complete() {
            ...
            if (self.numberOfSlides() !== 1 || self.isAnimatedOut) {
                ...
                Velocity(next.animateIn(), 'transition.slideUpIn');
            }
        ...
        if (this.numberOfSlides() === 1) {
            ...
            Velocity(this.getSlide().animateOut(), 'transition.slideUpOut', {
                complete: complete
            });
        }
	},
